    ...
    public function login() {
        if(isset($_POST["email_admin"])) {
            ...
            if($login->status == 200) {
                ...
                // Validar el estado del administrador

                if($login->results[0]->status_admin == 0) {
                    echo '<div class="alert alert-danger mt-3 rounded">Error al ingresar: Administrador no autorizado</div>
                    
                    <script>
                        fncMatPreloader("off");
                        fncFormatInputs();
                        fncToastr("error","Error al ingresar: Administrador no autorizado");
                    </script>';
